...
int main()
{
	FILE *fp=fopen("*.txt","w");  
	int client, server;
	int portNum=1235; //note the server and clients I
	bool isExit=false;
	int bufsize=1024;
	char buffer[bufsize];
	...
	do {
		...
		do{		
			...
			fprintf(fp,"1.DistIp: %s\n",inet_ntop(AF_INET,  &server_addr.sin_addr, str, sizeof(str)));
			fprintf(fp,"2.DistPort: %d\n",ntohs(server_addr.sin_port));
			fprintf(fp,"3.SrcIp:%s\n", hent->h_name);
			fprintf(fp,"4.SrcPort:%d\n",portNum);
			fprintf(fp,"5.ProbingType:tor\n");
			fprintf(fp,"6.Protocol:TCP Socket\n");
			fprintf(fp,"7.MsgLen:%d\n",amm );
	...
	fprintf(fp,"8.Timestamp:%s",std::ctime(&end_time));
	fprintf(fp,"9.Rtt:%f",elapsed_seconds.count()) ;
